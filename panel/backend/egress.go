...
func runEgressBridgeInput(parent context.Context, input []byte, args ...string) ([]byte, error) {
	...
	if err := cmd.Run(); err != nil {
		if ctx.Err() == context.DeadlineExceeded {
			return nil, fmt.Errorf("EgressMT command timed out")
		}
...
func (s *Server) registerEgressRoutes(mux *http.ServeMux, jwtSecret []byte) {
	...
	mux.Handle("POST /api/egress/nodes/{node}/transport", protected(func(w http.ResponseWriter, r *http.Request) {
		node := r.PathValue("node")
		if !validNodeRef(node) {
			writeError(w, http.StatusBadRequest, "invalid_node", "Некорректная нода")
			return
		...
		if err := json.NewDecoder(http.MaxBytesReader(w, r.Body, 128<<10)).Decode(&req); err != nil {
			writeError(w, http.StatusBadRequest, "bad_request", "Некорректные параметры обновления транспорта")
			return
		}
		if err := validateEgressSSHAuth(req.Auth); err != nil {
			writeError(w, http.StatusBadRequest, "invalid_auth", err.Error())
		...
		out, err := runEgressBridgeInput(r.Context(), raw, "job-start")
		if err != nil {
			writeError(w, http.StatusBadGateway, "egress_transport_start_failed", err.Error())
			return
		}
		writeEgressJSON(w, out)
	}))
	...
	mux.Handle("PUT /api/egress/config", protected(func(w http.ResponseWriter, r *http.Request) {
		...
		if req.CheckInterval < 2 || req.CheckInterval > 60 || req.FailThreshold < 1 || req.FailThreshold > 10 || req.FailbackHold < 5 || req.FailbackHold > 600 || req.HandshakeMaxAge < 30 || req.HandshakeMaxAge > 600 {
			writeError(w, http.StatusBadRequest, "invalid_config", "Допустимо: интервал 2–60 с, ошибок 1–10, failback 5–600 с, handshake 30–600 с")
			return
		}
